...
export async function POST(req, { params }) {
    ...
    if (action === "create") {
        ...
        const payload = {
            ...
            redirectUrl: `${req.headers.get('origin')}/userLogin/order-confirmation`,
            redirectMode: 'REDIRECT',
            callbackUrl: `${req.headers.get('origin')}/api/phonepe/verify`,
        ...
        try {
            const res = await axios.post(`${baseUrl}/pg/v1/pay`, { req: base64Payload }, {
                headers: {
            ...
            console.err('Error initiating PhonePe payment:', err);
    ...
    if (action === "verify") {
        ...
        try {
            const res = await axios.get(
                `${baseUrl}/pg/v1/status/${process.env.PHONEPE_MERCHANT_ID}/${merchantTransactionId}`, {
                    headers: {
                        'Content-Type': 'application/json',
                        'X-VERIFY': xVerify,
            ...
            console.err('Error verifying PhonePe payment:', err);
